...
module.exports = grammar({
  ...
  rules: {
    ...
    _type_params: ($) =>
      choice($._type_param, parenthesize(sep1(",", $._type_param))),
    ...
    _type_equation: ($) => seq(choice("=", ":="), optional("private"), $._type),
    ...
    _constructor_argument: ($) =>
      choice(sep1("*", $._simple_type), $.record_declaration),

    record_declaration: ($) =>
      seq("{", sep1(";", $.field_declaration), optional(";"), "}"),

    field_declaration: ($) =>
      seq(optional("mutable"), $._field_name, $._polymorphic_typed),
    ...
    _polymorphic_type: ($) => choice($.polymorphic_type, $._type),
    ...
    _abstract_type: ($) => seq("type", repeat1($._type_constructor)),
    ...
    _tuple_type: ($) => choice($._simple_type, $.tuple_type),

    _type: ($) => choice($._tuple_type, $.function_type, $.aliased_type),

    function_type: ($) =>
      prec.right(PREC.seq, seq(choice($.typed_label, $._type), "->", $._type)),

    typed_label: ($) =>
      prec.left(PREC.seq, seq(optional("?"), $._label_name, ":", $._type)),

    tuple_type: ($) => prec(PREC.prod, seq($._tuple_type, "*", $._simple_type)),

    constructed_type: ($) =>
      prec(
        PREC.app,
        seq(
          choice($._simple_type, parenthesize(sep1(",", $._type))),
          $.type_constructor_path,
        ),
      ),

    aliased_type: ($) => prec(PREC.match, seq($._type, "as", $.type_variable)),
    ...
    hash_type: ($) =>
      prec(
        PREC.hash,
        seq(
          optional(choice($._simple_type, parenthesize(sep1(",", $._type)))),
          "#",
          $.class_type_path,
        ),
      ),
    ...
    typed_expression: ($) =>
      parenthesize(seq($._sequence_expression, $._typed)),
    ...
    list_expression: ($) =>
      seq("[", optional(seq(sep1(";", $._expression), optional(";"))), "]"),
    ...
    coercion_expression: ($) =>
      parenthesize(
        seq($._sequence_expression, optional($._typed), ":>", $._type),
      ),
    ...
    alias_pattern: ($) =>
      prec.left(PREC.match, seq($._pattern, "as", $._value_pattern)),

    alias_binding_pattern: ($) =>
      prec.left(PREC.match, seq($._binding_pattern, "as", $._value_name)),

    typed_pattern: ($) => seq(parenthesize(seq($._pattern, $._typed))),

    typed_binding_pattern: ($) =>
      seq(parenthesize(seq(field("pattern", $._binding_pattern), $._typed))),

    or_pattern: ($) => prec.left(PREC.seq, seq($._pattern, "|", $._pattern)),

    or_binding_pattern: ($) =>
      prec.left(PREC.seq, seq($._binding_pattern, "|", $._binding_pattern)),
    ...
    constructor_binding_pattern: ($) =>
      prec.right(
        PREC.app,
        seq($.constructor_path, field("pattern", $._binding_pattern)),
      ),

    tag_pattern: ($) => prec.right(PREC.app, seq($.tag, $._pattern)),

    tag_binding_pattern: ($) =>
      prec.right(PREC.app, seq($.tag, field("pattern", $._binding_pattern))),

    polymorphic_variant_pattern: ($) => seq("#", $.type_constructor_path),

    tuple_pattern: ($) =>
      prec.left(PREC.prod, seq($._pattern, ",", $._pattern)),

    tuple_binding_pattern: ($) =>
      prec.left(PREC.prod, seq($._binding_pattern, ",", $._binding_pattern)),
    ...
    list_binding_pattern: ($) =>
      prec.left(
        seq(
          "[",
          optional(seq(sep1(";", $._binding_pattern), optional(";"))),
          "]",
        ),
      ),
    ...
    cons_binding_pattern: ($) =>
      prec.right(PREC.cons, seq($._binding_pattern, "::", $._binding_pattern)),
    ...
    lazy_binding_pattern: ($) =>
      prec(PREC.hash, seq("lazy", optional($._attribute), $._binding_pattern)),
    ...
    extended_module_path: ($) => "TODO: extended_module_path",
    // choice(
